...
# workaround for working with sparse_dot_mkl when only LP64 (32-bit) MKL interface available
# we split the matrix up into small enough (<2**31-1) sized blocks
# so it fits

def csr_blocks(A, max_nnz):
    if A.nnz < max_nnz:
        return [A]
    n = A.shape[0]
    ...
    while start_row < n:
        nnz_in_block = 0
        end_row = start_row
        while end_row < n and nnz_in_block + (indptr[end_row+1] - indptr[end_row]) <= max_nnz:
            nnz_in_block += indptr[end_row+1] - indptr[end_row]
            end_row += 1
...
def blocked_spmv(A_blocks, x):
    y = np.zeros(x.shape, dtype=x.dtype)
    start_row = 0
    for A_block in A_blocks:
        end_row = start_row + A_block.shape[0]
        y[start_row:end_row] = sparse_dot_mkl.sparse_dot.dot_product_mkl(A_block, x)
        start_row = end_row
    return y

def pyci_op_to_csr(op, max_nnz=2**31-1):
    indices = op.indices()
    indptr = op.indptr()
    data = op.data()
    matrix = scipy.sparse.csr_array((data, indices, indptr))
    print('INFO: nnz=', matrix.nnz)
    diagonal = matrix.diagonal()
    matrix += matrix.T
    matrix -= scipy.sparse.diags(diagonal)
    matrix_blocks = csr_blocks(matrix, max_nnz=max_nnz)
    matvec = lambda v: blocked_spmv(matrix_blocks, v)
    return matrix, diagonal, matvec
...
def resp(ham, nelec, gs_wfn, gs_evecs, integral, omega, gamma, model=5, triplet=False):
    ...
    if couple_property:
        ...
        t2 = time.time()
        print('timing::rsp-couple-property::add_hci', t2 - t1)
        t1 = time.time()
        op.update(ham, wfn)
        t2 = time.time()
        print('timing::rsp-couple-property::op_update', t2 - t1)
        t1 = time.time()
        e_vecs = np.concatenate((e_vecs, np.zeros((dets_added, e_vecs.shape[1]))), axis=0)
        matrix, hdiag, matvec = pyci_op_to_csr(op)
        t2 = time.time()
        print('timing::rsp-couple-property::op_transfer', t2 - t1)
        t1 = time.time()
        e_vals, e_vecs = solve_ci(matvec, hdiag, roots=1, c0=e_vecs, verbose=True)
        e_vals += op.ecore
        t2 = time.time()
        print('timing::rsp-couple-property::op_solve', t2 - t1)
        t1 = time.time()
        delta_e = old_energy - np.min(e_vals)
        old_energy = np.min(e_vals)
        num_determinants = e_vecs.shape[0]
    ...
    t2 = time.time()
    print('timing::rsp::davidson', t2 - t1)
    t1 = time.time()
    ...
    if couple_response:
        while True:
            ...
            t2 = time.time()
            print('timing::rsp-couple::add_hci', t2 - t1)
            t1 = time.time()
            op.update(ham, wfn)
            t2 = time.time()
            print('timing::rsp-couple::op_update', t2 - t1)
            t1 = time.time()
            #mu_op.update(ham_mu, wfn)
            e_vecs = np.concatenate((e_vecs, np.zeros((dets_added, e_vecs.shape[1]))), axis=0)
            matrix, hdiag, matvec = pyci_op_to_csr(op)
            t2 = time.time()
            print('timing::rsp-couple::op_transfer', t2 - t1)
            t1 = time.time()
            e_vals, e_vecs = solve_ci(matvec, hdiag, roots=1, c0=e_vecs, verbose=True)
            e_vals += op.ecore
            t2 = time.time()
            print('timing::rsp-couple::op_solve', t2 - t1)
            t1 = time.time()
            delta_e = old_energy - np.min(e_vals)
            old_energy = np.min(e_vals)
            num_determinants = e_vecs.shape[0]
            ...
            zPsi -= e_vecs[:,0] * np.dot(e_vecs[:,0], zPsi)
            t2 = time.time()
            print('timing::rsp-couple::zPsi', t2 - t1)
            t1 = time.time()
            E0 = np.dot(e_vecs[:,0], matvec(e_vecs[:,0]))
            E0w = E0 + omega + 1j * gamma
            ...
            t2 = time.time()
            print('timing::rsp-couple::davidson', t2 - t1)
            t1 = time.time()
...
def response_dot(response_wfn, response_vector, response_e_vecs, h1ao, triplet=None):
    ...
    property_vector = property_operator(response_wfn.to_det_array(), mu, response_e_vecs[0])
...
t2 = time.time()
print('timing::compute_rdms', t2 - t1)
t1 = time.time()
...
for ia in range(m.natm):
    if not any([ia in pair for pair in nuc_pair]):
        # skip response on this nucleus
        for i in range(3):
