...
if args.individual:
    for i, label in enumerate(labels):
        integrals = {label: dipole_integrals_mo[i]}
        perturbations = [(label, frequency, parity) for frequency in [0.0] 
                                                    for parity in [0]]
        resp_pt2(ham, wfn, op, e_vecs, integrals, perturbations, eps2, eps2mult, eps_mu=eps_mu, eps_resp=eps_resp, state=state, overwrite=False, reference_state=reference_state)
else:
    ...
    perturbations = [(label, frequency, parity) for label in labels 
                                                ...
                                                for parity in [0]]
    resp_pt2(ham, wfn, op, e_vecs, integrals, perturbations, eps2, eps2mult, eps_mu=eps_mu, eps_resp=eps_resp, state=state, reference_state=reference_state)
